...
local function loadAimbotGUI()
	...
	if CoreGui:FindFirstChild("UTS_CGE_Suite") then
		-- A simple notification function would be ideal here.
		-- Assuming one exists in the environment, otherwise print.
		if DoNotif then
	...
	pcall(function()
		...
		MainWindow.Size = UDim2.new(0, 520, 0, 420); -- Increased Height for new elements
		...
		TitleLabel.Text = "Gaming Chair v2.0";
		...
		do
			...
			local smoothingFactor = 0.2; -- Default alpha for Lerp
			...
			local minSmooth, maxSmooth = 0.05, 1.0; -- 1.0 is minimal smoothing
			...
			local function getClosestTargetInScope()
				...
				for _, model in ipairs(getgenv().TargetIndex) do
					if model and model.Parent then
						...
						if player and player == LocalPlayer then continue end -- Skip self
			...
			UserInputService.InputBegan:Connect(function(input, processed)
				...
				if (key == "MOUSEBUTTON2" and input.UserInputType == Enum.UserInputType.MouseButton2) or (input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode.Name:upper() == key) then
					aiming = true; FovCircle.Visible = true
				end
			end)
			...
			UserInputService.InputEnded:Connect(function(input)
				local key = toggleKeyBox.Text:upper();
				if (key == "MOUSEBUTTON2" and input.UserInputType == Enum.UserInputType.MouseButton2) or (input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode.Name:upper() == key) then
					aiming = false; FovCircle.Visible = false; clearESP()
				end
			end)
			
			-- Render Loop
			local currentTarget = nil
			RunService.RenderStepped:Connect(function(deltaTime) -- Pass deltaTime for physics calculations
				...
				if aiming and aimPart and targetModel then
					if not wallCheckEnabled or isPartVisible(aimPart) then
						...
						-- [ANTI-CHEAT] Add slight randomization to prediction timing to appear more human
						local predictionFactor = (distance / 2000) * (1 + (math.random(-50, 50) / 1000)) -- +/- 5% variance
						...
						elseif smoothingEnabled then
							-- [SMOOTHING] Use CFrame:Lerp for smoother, more human-like aiming
							local goalCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)
							-- Use delta time to make smoothing frame rate independent
			...
			task.spawn(function()
				local function RebuildTargetIndex()
					local newIndex = {}
					if not getgenv().TargetScope or not getgenv().TargetScope.Parent then getgenv().TargetScope = Workspace end -- Sanity check
				...
				while task.wait(2) and MainScreenGui.Parent do -- Stop loop when GUI is closed
					RebuildTargetIndex()
				end
			end)
