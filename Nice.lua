...
MODIFIED BY CALLUM - V42.4 - Patched weapon list population and added robust data-loading diagnostics.
...
-- GUI Instances
local screenGui = Instance.new("ScreenGui"); screenGui.Name = "MasterEquipperGUI_V42_4"; screenGui.ResetOnSpawn = false; screenGui.Parent = LOCAL_PLAYER:WaitForChild("PlayerGui")
...
local titleLabel = Instance.new("TextLabel", mainFrame); titleLabel.Size = UDim2.new(1, 0, 0, 30); titleLabel.BackgroundColor3 = Color3.fromRGB(40, 40, 40); titleLabel.Text = "Master Equipper V42.4 (Data-Loading Fix)"; titleLabel.Font = Enum.Font.SourceSansBold; titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255); titleLabel.TextSize = 18
...
local perkList = Instance.new("ScrollingFrame", configPanel); perkList.Size = UDim2.new(0.5, -10, 0, 200); perkList.Position = UDim2.new(0, 5, 0, 100); perkList.BackgroundColor3 = Color3.fromRGB(35,35,35); perkList.ScrollBarThickness = 4
local perkListLayout = Instance.new("UIListLayout", perkList); perkListLayout.Padding = UDim.new(0, 5)
local variantList = Instance.new("ScrollingFrame", configPanel); variantList.Size = UDim2.new(0.5, -10, 0, 200); variantList.Position = UDim2.new(0.5, 5, 0, 100); variantList.BackgroundColor3 = Color3.fromRGB(35,35,35); variantList.ScrollBarThickness = 4

-- Bottom Buttons
local equipButton = Instance.new("TextButton", mainFrame); equipButton.Name = "EquipButton"; equipButton.Size = UDim2.new(0.5, -135, 0, 30); equipButton.Position = UDim2.new(0, 260, 1, -35); equipButton.BackgroundColor3 = Color3.fromRGB(83, 12, 255); equipButton.Font = Enum.Font.SourceSansBold; equipButton.Text = "CONSTRUCT & EQUIP"; equipButton.TextColor3 = Color3.fromRGB(255, 255, 255); equipButton.TextSize = 16
local setKeybindButton = Instance.new("TextButton", mainFrame); setKeybindButton.Name = "SetKeybindButton"; setKeybindButton.Size = UDim2.new(0.5, -135, 0, 30); setKeybindButton.Position = UDim2.new(0.5, 95, 1, -35); setKeybindButton.BackgroundColor3 = Color3.fromRGB(50, 50, 50); setKeybindButton.Font = Enum.Font.SourceSansBold; setKeybindButton.Text = "SET KEYBIND"; setKeybindButton.TextColor3 = Color3.fromRGB(255, 255, 255); setKeybindButton.TextSize = 16

-- Keybinds Display
local keybindsDisplay = Instance.new("ScrollingFrame", configPanel); keybindsDisplay.Name = "KeybindsDisplay"; keybindsDisplay.Size = UDim2.new(1, -10, 0, 110); keybindsDisplay.Position = UDim2.new(0, 5, 1, -150); keybindsDisplay.BackgroundColor3 = Color3.fromRGB(35,35,35); keybindsDisplay.BorderColor3 = Color3.fromRGB(80, 80, 80);
local keybindsDisplayLayout = Instance.new("UIListLayout", keybindsDisplay); keybindsDisplayLayout.Padding = UDim.new(0, 3)
local keybindsTitle = Instance.new("TextLabel", configPanel); keybindsTitle.Size = UDim2.new(1, -10, 0, 20); keybindsTitle.Position = UDim2.new(0, 5, 1, -175); keybindsTitle.Text = "Active Keybinds"; keybindsTitle.Font = Enum.Font.SourceSansBold; keybindsTitle.TextColor3 = Color3.fromRGB(220,220,220); keybindsTitle.TextXAlignment = Enum.TextXAlignment.Left; keybindsTitle.BackgroundTransparency = 1;
local rarityDropdown, variantDropdown;


-- MODULES AND FUNCTIONS (Slider, Dropdown, etc.)
-- ... (The createSlider and dropdownModule functions are correct and unchanged) ...
...
-- --> MOD: This entire function has been rebuilt for robustness.
local function initializeFirstTime()
    if isInitialized then return true end
    print("--> V42.4: Initializing...")

    local modulesFolder = ReplicatedStorage:FindFirstChild("modules")
    if not modulesFolder then
        warn("--> CRITICAL FAILURE: 'modules' folder not found. Cannot load weapon data.")
        return false
    end
    
    -- Load item data with proper error handling
    local success, result = pcall(function() return require(modulesFolder:WaitForChild("itemData")) end)
    if success then
        masterWeaponMap = result
        print("--> Successfully loaded itemData module.")
    else
        warn("--> CRITICAL FAILURE: Could not require 'itemData' module. The weapon list will be empty. Error:", result)
        return false
    end

    if not next(masterWeaponMap) then
        warn("--> WARNING: itemData loaded, but the masterWeaponMap is empty. The weapon list will be blank.")
    end
    ...
    local sortedWeapons = {}; for n, d in pairs(masterWeaponMap) do if type(d) == "table" and d.itemType == "weapons" then table.insert(sortedWeapons, n) end end; table.sort(sortedWeapons)
    
    print(`--> Found { #sortedWeapons } weapons to populate the list.`)
    if #sortedWeapons == 0 then
        warn("--> WARNING: No items with itemType 'weapons' found in itemData. List will be blank.")
    end
    ...
    for _, name in ipairs(sortedWeapons) do
        ...
        btn.MouseButton1Click:Connect(function() selectedWeapon=name; selectedWeaponLabel.Text="Selected: "..name; for _,s in pairs(perkControls) do s:SetValue(0) end; variantDropdown:UpdateOptions({"None"}); if variantDatabase[name] then variantDropdown:UpdateOptions(table.insert(variantDatabase[name], 1, "None") and variantDatabase[name]) end end)
    end
    ...
    print("--> V42.4: Database built and GUI populated."); isInitialized = true; return true
end

-- All other functions (updateKeybindsDisplay, executeEquipSequence, connections for buttons, etc.) are unchanged.
-- I'm including them again for a complete, copy-pasteable script.

local function updateKeybindsDisplay()
    for _, child in ipairs(keybindsDisplay:GetChildren()) do if not child:IsA("UILayout") then child:Destroy() end end
    for keyCode, config in pairs(weaponKeybinds) do
        local keybindLabel = Instance.new("TextLabel"); keybindLabel.Size = UDim2.new(1, -10, 0, 20); keybindLabel.Text = `  [ {keyCode.Name} ] - {config.weaponName}`; keybindLabel.Font = Enum.Font.SourceSans; keybindLabel.TextColor3 = Color3.fromRGB(200, 200, 200); keybindLabel.TextXAlignment = Enum.TextXAlignment.Left; keybindLabel.BackgroundColor3 = Color3.fromRGB(50, 50, 50); keybindLabel.Parent = keybindsDisplay
    end
    task.wait(); keybindsDisplay.CanvasSize = UDim2.fromOffset(0, keybindsDisplayLayout.AbsoluteContentSize.Y)
end

local function verifyWeaponConstruction(weaponName)
    print(`--> Verifying construction of '{weaponName}'...`)
    local playerData = LOCAL_PLAYER:WaitForChild("data", 5); if not playerData then warn("--> Verification FAILED: Player 'data' folder did not appear."); return end
    local exaltedWeapons = playerData:WaitForChild("exaltedWeapons", 2); if not exaltedWeapons then warn("--> Verification FAILED: 'exaltedWeapons' table did not appear."); return end
    if exaltedWeapons:FindFirstChild(weaponName) then print("--> SUCCESS: Construction confirmed.") else warn("--> FAILED: Weapon entry did not appear in local data.") end
end
...
local function executeEquipSequence(config)
    ...
    local weaponData = {perkData = {}, rarity = config.rarity, level = config.level, attributes = {}}; pcall(function() remotes.reroll:InvokeServer(config.weaponName, weaponData) end); task.wait(0.2); pcall(function() remotes.equip:InvokeServer(config.weaponName) end); task.wait(0.3)
    for perkName, perkValue in pairs(config.perks) do for i = 1, perkValue do pcall(function() remotes.upgrade:FireServer(config.weaponName, perkName) end); task.wait(0.05) end end
    if config.variant and config.variant ~= "None" then pcall(function() remotes.infuse:FireServer(config.weaponName, config.variant) end); print(`--> Fired Infuse for variant: "{config.variant}"`) end
    print("--> Injection sequence complete."); task.delay(1, function() verifyWeaponConstruction(config.weaponName) end)
end
...
setKeybindButton.MouseButton1Click:Connect(function()
    if not isInitialized or not selectedWeapon or isSettingKeybind then return end; isSettingKeybind = true; setKeybindButton.Text = "PRESS ANY KEY..."; setKeybindButton.BackgroundColor3 = Color3.fromRGB(200, 100, 0)
    local connection; connection = UserInputService.InputBegan:Connect(function(input, gpe)
        if gpe or input.UserInputType ~= Enum.UserInputType.Keyboard then return end
        if input.KeyCode == Enum.KeyCode.RightControl then warn("--> Cannot bind to the GUI toggle key."); connection:Disconnect(); setKeybindButton.Text = "SET KEYBIND"; setKeybindButton.BackgroundColor3 = Color3.fromRGB(50, 50, 50); isSettingKeybind = false; return end
        local currentConfig = { weaponName = selectedWeapon, rarity = rarityDropdown.GetSelectedOption(), level = math.clamp(tonumber(levelInput.Text) or 800, 1, 800), variant = variantDropdown:GetSelectedOption(), perks = {} }; for perkName, slider in pairs(perkControls) do currentConfig.perks[perkName] = slider:GetValue() end
        weaponKeybinds[input.KeyCode] = currentConfig; print(`--> Keybind set! Key '{input.KeyCode.Name}' is now bound to '{selectedWeapon}'.`); updateKeybindsDisplay(); connection:Disconnect(); setKeybindButton.Text = "SET KEYBIND"; setKeybindButton.BackgroundColor3 = Color3.fromRGB(50, 50, 50); isSettingKeybind = false
    end)
end)

equipButton.MouseButton1Click:Connect(function()
    if not isInitialized then if not initializeFirstTime() then warn("Init failed."); return end end; if not selectedWeapon then return end
    local character = LOCAL_PLAYER.Character; local humanoid = character and character:FindFirstChildOfClass("Humanoid")
    local currentConfig = { weaponName = selectedWeapon, rarity = rarityDropdown.GetSelectedOption(), level = math.clamp(tonumber(levelInput.Text) or 800, 1, 800), variant = variantDropdown:GetSelectedOption(), perks = {} }; for perkName, slider in pairs(perkControls) do currentConfig.perks[perkName] = slider:GetValue() end
    if humanoid and humanoid.Health > 0 then print("--> Player is alive in-round. Equipping immediately."); executeEquipSequence(currentConfig) else queuedWeaponConfig = currentConfig; print(`--> Player is in lobby. Queued '{selectedWeapon}' for next spawn.`); selectedWeaponLabel.Text = "QUEUED: " .. selectedWeapon end
end)

LOCAL_PLAYER.CharacterAdded:Connect(function(character)
    if queuedWeaponConfig then print("--> Character spawned, proceeding with queued weapon equip."); local configToEquip = queuedWeaponConfig; queuedWeaponConfig = nil; task.wait(2); executeEquipSequence(configToEquip) end
end)

UserInputService.InputBegan:Connect(function(input, gpe)
    if gpe then return end; if input.KeyCode == Enum.KeyCode.RightControl then mainFrame.Visible = not mainFrame.Visible; if mainFrame.Visible and not isInitialized then initializeFirstTime() end; return end
    if isSettingKeybind then return end; local boundConfig = weaponKeybinds[input.KeyCode]
    if boundConfig then print(`--> Keybind '{input.KeyCode.Name}' pressed.`); local character = LOCAL_PLAYER.Character; local humanoid = character and character:FindFirstChildOfClass("Humanoid")
        if humanoid and humanoid.Health > 0 then print("--> Executing keybind sequence."); executeEquipSequence(boundConfig) else warn("--> Cannot equip via keybind while in the lobby or dead.") end
    end
end)
