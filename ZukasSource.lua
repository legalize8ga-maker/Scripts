...
function Modules.ItemResurrection:Enable(): ()
    if self.State.IsEnabled then return end
    if not (getrawmetatable and setreadonly and newcclosure) then 
        return DoNotif("Executor does not support __gc hooks.", 4) 
    end

    -- Create a secure cache for resurrected tools
    self.State.CacheFolder = Instance.new("Folder")
    self.State.CacheFolder.Name = "ResurrectionCache_" .. math.random(1e5, 1e6)
    self.State.CacheFolder.Parent = self.Services.CoreGui

    -- We must get the metatable from a live instance. We create a temporary one to do this.
    local tempTool = Instance.new("Tool")
    local mt = getrawmetatable(tempTool)
    tempTool:Destroy() -- Clean up the temporary tool

    if not mt then return DoNotif("Failed to acquire Tool metatable.", 4) end

    self.State.ToolMetatable = mt
    self.State.OriginalGC = mt.__gc
    local cache = self.State.CacheFolder

    setreadonly(mt, false)
    mt.__gc = newcclosure(function(toolInstance: Tool)
        -- This function runs on the object right before it's destroyed.
        -- By re-parenting it, we cancel the garbage collection.
        pcall(function()
            if toolInstance:IsA("Tool") and toolInstance:FindFirstChild("Handle") then
                warn("--> [ItemResurrection] Intercepted GC call for Tool: " .. toolInstance.Name .. ". Resurrecting.")
                toolInstance.Parent = cache
            end
        end)
        -- We do not call the original __gc, as that would proceed with the destruction.
    end)
    setreadonly(mt, true)

    self.State.IsEnabled = true
    DoNotif("Item Resurrection: ENABLED. Dropped/destroyed tools will be cached.", 3)
end

function Modules.ItemResurrection:Disable(): ()
    if not self.State.IsEnabled then return end
    
    if self.State.ToolMetatable and self.State.OriginalGC then
        pcall(function()
            setreadonly(self.State.ToolMetatable, false)
            self.State.ToolMetatable.__gc = self.State.OriginalGC
            setreadonly(self.State.ToolMetatable, true)
        end)
    end

    if self.State.CacheFolder then
        self.State.CacheFolder:Destroy()
    end
    
    self.State.IsEnabled = false
    self.State.OriginalGC = nil
    self.State.ToolMetatable = nil
    self.State.CacheFolder = nil
    DoNotif("Item Resurrection: DISABLED. Cache cleared.", 2)
end

function Modules.ItemResurrection:Initialize(): ()
    RegisterCommand({
        Name = "autokeep",
        Aliases = {"antideleteitem", "resurrect"},
        Description = "Toggles a __gc hook to prevent your tools from being deleted."
    }, function()
        if self.State.IsEnabled then self:Disable() else self:Enable() end
    end)
    ...
    RegisterCommand({
        Name = "showcache",
        Aliases = {"listcache"},
        Description = "Lists all resurrected tools currently held in the cache."
    }, function()
        if not self.State.IsEnabled or not self.State.CacheFolder then
            return DoNotif("Item Resurrection is not enabled.", 3)
        end
        local items = {}
        for _, item in ipairs(self.State.CacheFolder:GetChildren()) do
            table.insert(items, item.Name)
        end
        if #items == 0 then
            DoNotif("The resurrection cache is empty.", 2)
        else
            DoNotif("Cached Items: " .. table.concat(items, ", "), 5)
        end
    end)

    RegisterCommand({
        Name = "gettool",
        Aliases = {"getitem", "restoreitem"},
        Description = "Retrieves a specific tool from the resurrection cache."
    }, function(args)
        if not self.State.IsEnabled or not self.State.CacheFolder then
            return DoNotif("Item Resurrection is not enabled.", 3)
        end
        local toolName = args[1]
        if not toolName then return DoNotif("Usage: ;gettool <ToolName>", 3) end

        local backpack = self.Services.Players.LocalPlayer:FindFirstChildOfClass("Backpack")
        if not backpack then return DoNotif("Backpack not found.", 3) end

        local cachedTool = self.State.CacheFolder:FindFirstChild(toolName, true)
        if cachedTool and cachedTool:IsA("Tool") then
            cachedTool.Parent = backpack
            DoNotif("Restored '" .. cachedTool.Name .. "' to your backpack.", 2)
        else
            DoNotif("Tool '" .. toolName .. "' not found in cache.", 3)
        end
    end)
end
...
Modules.PhysicsThrottler = {
    State = {
        IsEnabled = false,
        OriginalNewIndex = nil,
        BasePartMetatable = nil
    },
    Services = {
        Players = game:GetService("Players")
    }
...
function Modules.PhysicsThrottler:Enable(): ()
    if self.State.IsEnabled then return end
    if not (getrawmetatable and setreadonly and newcclosure) then 
        return DoNotif("Executor does not support __newindex hooks.", 4) 
    end

    local tempPart = Instance.new("Part")
    local mt = getrawmetatable(tempPart)
    tempPart:Destroy()

    if not mt then return DoNotif("Failed to acquire BasePart metatable.", 4) end

    self.State.BasePartMetatable = mt
    self.State.OriginalNewIndex = mt.__newindex
    local original_ni = self.State.OriginalNewIndex
    local localPlayer = self.Services.Players.LocalPlayer

    setreadonly(mt, false)
    mt.__newindex = newcclosure(function(self: BasePart, key: string, value: any)
        if key == "CFrame" or key == "Velocity" then
            local player = self.Services.Players:GetPlayerFromCharacter(self:FindFirstAncestorOfClass("Model"))
            if player and player ~= localPlayer then
                -- Silently discard the physics update for all other players
                return
            end
        end
        return original_ni(self, key, value)
    end)
    setreadonly(mt, true)

    self.State.IsEnabled = true
    DoNotif("Physics Throttler: ENABLED. Other players are now frozen on your client.", 3)
end

function Modules.PhysicsThrottler:Disable(): ()
    if not self.State.IsEnabled then return end

    if self.State.BasePartMetatable and self.State.OriginalNewIndex then
        pcall(function()
            setreadonly(self.State.BasePartMetatable, false)
            self.State.BasePartMetatable.__newindex = self.State.OriginalNewIndex
            setreadonly(self.State.BasePartMetatable, true)
        end)
    end

    self.State.IsEnabled = false
    self.State.OriginalNewIndex = nil
    self.State.BasePartMetatable = nil
    DoNotif("Physics Throttler: DISABLED. Player synchronization restored.", 2)
end

function Modules.PhysicsThrottler:Initialize(): ()
    RegisterCommand({
        Name = "freezeplayers",
        Aliases = {"starplat", "throttle"},
        Description = "Toggles a physics hook to freeze other players on your screen."
    }, function()
        if self.State.IsEnabled then self:Disable() else self:Enable() end
    end)
end
...
function Modules.CameraLock:Enable(): ()
    if self.State.IsEnabled then return end
    if not (getrawmetatable and setreadonly) then return DoNotif("Executor does not support __newindex hooks.", 4) end

    local camera = self.Services.Workspace.CurrentCamera
    local mt = getrawmetatable(camera)
    if not mt then return DoNotif("Could not get camera metatable.", 3) end

    self.State.CameraMetatable = mt
    self.State.OriginalNewIndex = mt.__newindex
    local original_ni = self.State.OriginalNewIndex
    local localPlayer = self.Services.Players.LocalPlayer
    ...
    mt.__newindex = newcclosure(function(self, key, value)
        if key == "CameraSubject" then
            if value ~= localPlayer.Character and value ~= localPlayer.Character:FindFirstChild("Humanoid") then
                warn("--> [CameraLock] Blocked attempt to change CameraSubject to: " .. tostring(value))
                return -- Silently discard the write operation
            end
        end
        return original_ni(self, key, value)
    end)
    ...
    self.State.IsEnabled = true
    DoNotif("Camera Lock: ENABLED", 2)
end

function Modules.CameraLock:Disable(): ()
    if not self.State.IsEnabled then return end
    if self.State.CameraMetatable and self.State.OriginalNewIndex then
        pcall(function()
            setreadonly(self.State.CameraMetatable, false)
            self.State.CameraMetatable.__newindex = self.State.OriginalNewIndex
            setreadonly(self.State.CameraMetatable, true)
        end)
    end
    self.State.CameraMetatable = nil
    self.State.OriginalNewIndex = nil
    self.State.IsEnabled = false
    DoNotif("Camera Lock: DISABLED", 2)
end

function Modules.CameraLock:Initialize(): ()
    RegisterCommand({
        Name = "camlock",
        Aliases = {"antispectate"},
        Description = "Toggles a lock on the camera subject, preventing spectate/cutscenes."
    }, function()
        if self.State.IsEnabled then self:Disable() else self:Enable() end
    end)
end

Modules.RetaliatoryFling = {
    State = {
        IsEnabled = false,
        OriginalNamecall = nil
    },
...
function Modules.RetaliatoryFling:Enable(): ()
    if self.State.IsEnabled then return end
    if not (getrawmetatable and getnamecallmethod) then return DoNotif("Executor does not support __namecall hooks.", 4) end
    ...
    mt.__namecall = newcclosure(function(...)
        ...
        if method == "BreakJoints" and selfArg:IsA("Humanoid") and selfArg.Parent == localPlayer.Character then
            warn("--> [RetaliatoryFling] Blocked BreakJoints() call. Initiating counter-attack.")
            
            task.spawn(function()
                local myRoot = selfArg.RootPart
                if not myRoot then return end
                
                local closestPlayer, minDist = nil, 100 
                for _, player in ipairs(self.Services.Players:GetPlayers()) do
                    if player ~= localPlayer and player.Character then
                        local targetRoot = player.Character:FindFirstChild("HumanoidRootPart")
                        if targetRoot then
                ...
                if closestPlayer and closestPlayer.Character then
                    ...
                    if targetRoot then
                        ...
                        flingForce.MaxForce = Vector3.new(0, 1e9, 0)
                        flingForce.Velocity = Vector3.new(0, 250, 0)
                        flingForce.Parent = targetRoot
                        DoNotif("Retaliating against: " .. closestPlayer.Name, 1.5)
                        task.delay(0.2, function()
                            pcall(flingForce.Destroy, flingForce)
                        end)
                    end
                end
            end)
            
            return -- Swallow the BreakJoints call
        end
    ...
    self.State.IsEnabled = true
    DoNotif("Retaliatory Fling: ENABLED", 2)
end

function Modules.RetaliatoryFling:Disable(): ()
    if not self.State.IsEnabled then return end
    if self.State.OriginalNamecall then
        pcall(function()
            local mt = getrawmetatable(game)
    ...
    self.State.OriginalNamecall = nil
    self.State.IsEnabled = false
    DoNotif("Retaliatory Fling: DISABLED", 2)
end

function Modules.RetaliatoryFling:Initialize(): ()
    RegisterCommand({
        Name = "retaliate",
        Aliases = {"antiragdollfling", "revenge"},
        Description = "Toggles an anti-ragdoll that flings the nearest enemy on activation."
    }, function()
        if self.State.IsEnabled then self:Disable() else self:Enable() end
    end)
end
